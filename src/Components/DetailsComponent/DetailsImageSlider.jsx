...
export default function DetailsImageSlider({
  ...
  const handleImageClick = (state, index) => {
    handleShowImage(
      details?.package_files.filter((_, idx) => idx === index)[0]?.file_url
    );
    if (state) {
    ...
    <div className=" space-y-4">
      <div className="w-full flex justify-between">
        ...
        <div className="flex items-center gap-3">
          ...
          <div className="cursor-pointer">{loveIcon}</div>
        </div>
      </div>
              ...
              <CarouselContent className="flex gap-2">
                {details?.package_files?.map((planimg, index) => (
                  <CarouselItem key={planimg.id} className=" basis-1/5">
                    <div
                      onClick={() => handleImageClick(false, index)}
