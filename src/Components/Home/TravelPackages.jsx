...
const TravelPackages = () => {
    ...
    return (
        <div>
            {homeData?.packages?.length > 0 && <div className='max-w-[1216px] mx-auto px-5 2xl:px-0'>
                <div className='text-2xl md:text-5xl font-bold text-center'>Exclusive Travel Packages</div>
                <div className=' grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 justify-center gap-5 mb-8 py-12'>
                    {homeData?.packages?.map(tour => (
                        <Link to={`/packages/${tour?.id}`} key={tour?.id}>
                            <div className="relative flex flex-col my-6 bg-white group shadow-md hover:shadow-lg hover:border-orange-500 transform duration-300 border border-slate-200 rounded-lg h-full justify-between">
                                <div>
